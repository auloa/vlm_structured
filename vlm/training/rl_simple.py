...
def train_rl_simple_pg(
    ...
    assert completions_per_image >= 2, (
        "completions_per_image must be >= 2 because this loop uses "
    ...
    for epoch in range(epochs):
        ...
        for step, sample in enumerate(loader):
            ...
            # ------------------------------------------------------------
            # 1. Sample K completions from current policy.
            # ------------------------------------------------------------
            ...
            # Optional safety: if all rewards are identical, there is no
            # useful preference signal. The KL term alone should not train.
            if torch.allclose(advantages, torch.zeros_like(advantages), atol=1e-6):
                global_step += 1
            ...
            # ------------------------------------------------------------
            # 4. Reference log-probs from frozen SFT projector.
            # ------------------------------------------------------------
            with torch.no_grad():
            ...
            # ------------------------------------------------------------
            # 5. Current policy log-probs with gradient.
            # ------------------------------------------------------------
            with get_autocast(device):
                ...
                loss, policy_loss, kl_loss = compute_sequence_pg_kl_loss(
    ...
    writer.close()
    print("simple policy-gradient RL training complete")
...
def compute_sequence_pg_kl_loss(
    ...
    """
    Simple group-relative policy-gradient loss with KL regularization.
    ...
    Objective:
        - Increase log-prob of completions with positive advantage.
        - Decrease log-prob of completions with negative advantage.
        - Penalize drift from the frozen SFT reference projector.
    """
    ...
    # Average log-prob per completion, normalized by length.
    ...
    #
    # If advantage > 0:
    #   minimize -advantage * log_prob -> increase log_prob
    #
    # If advantage < 0:
    #   minimize -advantage * log_prob -> decrease log_prob
    policy_loss = -(advantages * policy_seq_log_probs).mean()

    # Non-negative sampled KL-style penalty.
    #
    # This is more stable than:
    #   policy_seq_log_probs - ref_seq_log_probs
    #
    # because this form is always >= 0.
...
def _save_simple_pg_checkpoint(
    model,
    optimizer,
    epoch: int,
    step: int,
